...
import { Server, Database, Cloud, Network, HardDrive, Shield, ShieldAlert, Box, Globe } from 'lucide-react';
...
const getResourceIcon = (resourceType?: string) => {
  ...
  if (rt.includes('vpc') || rt.includes('subnet') || rt.includes('route') || rt.includes('network')) {
    return <Network className="w-3.5 h-3.5 text-sky-400" />;
  }
  if (rt.includes('db') || rt.includes('dynamo') || rt.includes('rds') || rt.includes('sql')) {
    return <Database className="w-3.5 h-3.5 text-purple-400" />;
  }
  if (rt.includes('instance') || rt.includes('ecs') || rt.includes('lambda') || rt.includes('machine') || rt.includes('eks')) {
...
export const InfrastructureNode: React.FC<NodeProps> = memo(({ data, selected }) => {
  ...
  if (isContainer) {
    ...
    const isSubnet = resourceType === 'aws_subnet' || resourceType === 'azurerm_subnet';
    const isPublic = nodeData?.attributes?.subnet_type === 'public';

    let containerStyle = styles.cardBorder;
    if (isVPC) {
      containerStyle = 'border-2 border-sky-500/60 bg-slate-900/40 rounded-3xl shadow-2xl';
    } else if (isAZ) {
      containerStyle = 'border-2 border-dashed border-amber-500/50 bg-slate-800/25 rounded-2xl shadow-inner';
    } else if (isSubnet) {
      containerStyle = isPublic
        ? 'border-2 border-sky-400/60 bg-sky-950/30 rounded-xl shadow-lg'
        : 'border-2 border-emerald-500/50 bg-emerald-950/20 rounded-xl shadow-lg';
    } else if (isModule) {
    ...
    return (
      <div
        className={`relative w-full h-full min-w-[280px] min-h-[160px] transition-all duration-200 p-3 flex flex-col justify-between cursor-pointer ${
          containerStyle
        } ${
          selected
      ...
      >
        ...
        {/* Footer Badge */}
        <div className="flex items-center justify-between text-[9px] text-slate-500 font-mono pt-2 pointer-events-none">
          <span>
            {isModule
              ? 'Module Subgraph'
              : isAZ
          ...
          {nodeData?.attributes?.cidr_block ? (
            <span className="text-slate-400 bg-slate-900/90 px-1.5 py-0.2 rounded border border-slate-800 font-mono text-[8px]">
              {String(nodeData.attributes.cidr_block)}
            </span>
          ) : isAZ && nodeData?.attributes?.availability_zone ? (
            <span className="text-amber-300 bg-amber-950/80 px-1.5 py-0.2 rounded border border-amber-800/60 font-mono text-[8px]">
              {String(nodeData.attributes.availability_zone)}
            </span>
          ) : isModule && nodeData?.attributes?.source ? (
            <span className="text-indigo-300 bg-indigo-950/80 px-1.5 py-0.2 rounded border border-indigo-800/60 font-mono text-[8px]">
  ...
  // Case 3: Standard Resource Card (Fixed 240px x 75px)
  return (
    <div
